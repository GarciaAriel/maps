...
public class getRouteTask2 {
	...
	public ArrayList<LatLng> get_route(LatLng fromP,LatLng toP,LatLng bloqueo)
	{
		...
		if (respuesta != null){
			...
			double ParaSumar= (fromP.latitude-toP.latitude)/10;
			double newX = bloqueo.latitude+ParaSumar;
			double newY = ((pendiente*newX)+cons);
			
			while((v2GetRouteDirection.CalculationByDistance(bloqueo.latitude,bloqueo.longitude,newX,newY))<200)
			{
				newX = newX+ParaSumar;
			...
			res = get_route(fromP, medio, bloqueo);
